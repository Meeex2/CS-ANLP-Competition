...
#SBATCH --mem=32G
#SBATCH --output=nlp_kaggle_roberta_cont.out       # Standard output (with job ID)
#SBATCH --error=nlp_kaggle_roberta_cont.err        # Standard error (with job ID)
...
nvidia-smi -l 60 > gpu_usage_cnt.log 2>&1 &
MONITOR_PID=$!

# Run the training script with accelerate using 2 processes.
accelerate launch --multi_gpu --mixed_precision "fp16" --num_processes 4 scripts/continue_model.py > nlp_kaggle_roberta_cont_${SLURM_JOB_ID}.log
